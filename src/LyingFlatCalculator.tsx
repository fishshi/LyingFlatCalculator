...
function LyingFlatCalculator(): JSX.Element {
  ...
  const [balanceInput, setBalanceInput] = usePersistentState<string>("balanceInput", "3000000");
  useEffect(() => {
  ...
  const [expenseInput, setExpenseInput] = usePersistentState<string>("expenseInput", "60000");
  useEffect(() => {
  ...
  const [inflationRateInput, setInflationRateInput] = usePersistentState<string>("inflationRateInput", "3.00");
  useEffect(() => {
  ...
  const [investmentReturnRate, setInvestmentReturnRate] = useState<number | null>(2.0);
  const [investmentReturnRateInput, setInvestmentReturnRateInput] = usePersistentState<string>(
    "investmentReturnRateInput",
    "2.00"
  );
  useEffect(() => {
    if (investmentReturnRateInput == "" || isNaN(Number(investmentReturnRateInput))) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div className="flex flex-grow justify-between">
        ...
        <a href="https://github.com/fishshi/LyingFlatCalculator" target="_blank" rel="noopener noreferrer">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 p-6 sm:p-8">
        <NumberInput label="初始资金" value={balanceInput} onChange={setBalanceInput} title="初始资金" step={10000} />
        <NumberInput
          label="年支出（初始）"
          value={expenseInput}
      ...
      <div className="px-6 sm:px-8">
        <Chart balance={balance} expense={expense} inflationRate={inflationRate} investmentReturnRate={investmentReturnRate} />
      </div>
    </div>
  );
}
